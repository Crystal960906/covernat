...
//장바구니
import { addItem } from '../components/store.tsx'
import { useDispatch } from 'react-redux'
import { Button } from 'react-bootstrap'
import { Link } from 'react-router-dom';

interface SubImgDetail {
    imgDetail: string;
}
...
//장바구니 (유효성체크)
    const selproduct = props.item1.find((x) => x.id === Number(id));
    ...
    <div className="sub_wrap">
        ...
        <div className="sub_left">
            <div className='smallimgbox'>
                {selproduct?.SubImgDetail_set.map((detail, index) => (
                    <img key={index} src={process.env.PUBLIC_URL + detail.imgDetail} alt={`Detail ${index + 1}`} />
                ))}
            </div>
        </div>
        ...
        <Swiper scrollbar={{}} modules={[Scrollbar]} className="mySwiper">
            {selproduct?.SubImgDetail_set.map((imgDetail, index) => (
                <SwiperSlide key={index}>
                    <img src={process.env.PUBLIC_URL + imgDetail.imgDetail} alt="" />
                </SwiperSlide>
            ))}
            </Swiper>
            ...
            <TabContent tap={tap}/>
            <div>
                <img src={process.env.PUBLIC_URL + (selproduct?.imgPage || '')} style={{ width: "100%", marginTop: '30px' }} />
            </div>
        </div>
        {/* 오른쪽 시작==> */}
        <div className="sub_right">
            ...
            <table className="itemDetail_img">
                <tr>
                <td>
                <img src={process.env.PUBLIC_URL + selproduct?.optionImg[0].optionDetail} alt="" />
                <img src={process.env.PUBLIC_URL + selproduct?.optionImg[1].optionDetail} alt="" />
                </td>
                </tr>
                <tr>
    ...
    if (props.tap === 0) {
        ...
        <div className="reviewsWrap">
            ...
            <div className="reviews_icontent">
                <div className="score">
                    <img src={process.env.PUBLIC_URL + '/img/sub_page/star.png'} className='star' style={{width:'55px'}} />
                    <span>0.0</span>
                </div>
                ...
                <table>
                    <tr>
                        ...
                        <td className='qAnda_title'>재고조회
                            <img src={process.env.PUBLIC_URL + '/img/sub_page/ico_lock.gif'}  style={{padding:'7px'}}/>
                        </td>
                        <td className='qAnda_name'>김**</td>
                        <td className='qAnda_date'>2024-01-20</td>
                    </tr>
                    <tr>
                    <td className='qAnda_num'>2</td>
                        <td className='qAnda_title'>반품/교환
                            <img src={process.env.PUBLIC_URL + '/img/sub_page/ico_lock.gif'}  style={{padding:'7px'}}/>
                        </td>
                        <td className='qAnda_name'>조**</td>
                        <td className='qAnda_date'>2024-02-28</td>
                    </tr>
                </table>
            </div>
        )
